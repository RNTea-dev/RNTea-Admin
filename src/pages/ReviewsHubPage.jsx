...
const ReviewsHubPage = () => {
    ...
    const loadHospitals = useCallback(async () => {
        console.log("ReviewsHubPage: loadHospitals called. AuthReady:", authReady);
        if (!authReady || !db || !appId) {
            showMessage('Firebase not ready or app ID missing. Please wait.', 'error');
            return;
        }
        ...
        try {
            ...
            const querySnapshot = await getDocs(hospitalsColRef);
            ...
            console.log("ReviewsHubPage: Fetched Hospitals:", fetchedHospitals.length);

            setHospitals(fetchedHospitals);
            setCurrentHospitalDisplayCount(20);
            setLoadingHospitals(false);
            setNoHospitalsFound(fetchedHospitals.length === 0);
        } catch (error) {
            console.error("ReviewsHubPage: Error fetching hospitals:", error);
    ...
    // Initial load of hospitals when component mounts or authReady changes
    useEffect(() => {
        console.log("ReviewsHubPage: Initial useEffect for loadHospitals. AuthReady:", authReady);
        if (authReady) {
            loadHospitals();
        }
    }, [authReady, loadHospitals]);
    ...
    const handleHospitalSelect = useCallback(async (hospital) => {
        ...
        try {
            ...
            const querySnapshot = await getDocs(doctorsColRef);
            ...
            console.log("ReviewsHubPage: Fetched Doctors for selected hospital:", fetchedDoctors.length);
            ...
            console.error("ReviewsHubPage: Error fetching doctors:", error);
                ...
                <section id="doctor-selection-section" ref={doctorSelectionSectionRef} className={`bg-white p-6 md:p-8 rounded-lg shadow-lg mb-8 section-hover scroll-margin-top-adjusted ${selectedHospital ? '' : 'hidden'}`}>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-4 mb-6">
                        ...
                        <button
                            ...
                            className="bg-custom-beige text-gray-800 font-bold py-3 px-6 rounded-full hover:opacity-90 transition duration-300 shadow-md w-full md:w-auto btn-hover-scale"
                            onClick={filterAndRenderDoctors}
                        >
                            Search Doctor
                        </button>
                    </div>
